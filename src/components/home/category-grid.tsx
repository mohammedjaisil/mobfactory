 ...
 * section scrolls, settles into an even 3×3 grid where every tile is the same
 * size AND fully visible (the grid is sized to the viewport height, so no rows
 * clip). Mobile / reduced-motion get a plain grid.
 */
const TILES = [
...
function Tile({ t, priority, sizes }: { t: (typeof TILES)[number]; priority?: boolean; sizes: string }) {
  return (
    <Link
      href={t.href}
...
export function CategoryGrid() {
  ...
  // Featured settles to 1 by ~55%; the rest of the scroll shows the finished grid.
  const featuredScale = useTransform(scrollYProgress, [0, 0.55], [3.4, 1]);
      ...
      <section className="bg-[#07081a] py-16 text-white">
        <div className="mx-auto max-w-[1680px] px-4 sm:px-6">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 sm:gap-3">
            {TILES.map((t, i) => (
              <div key={t.label} className="aspect-[16/10]">
                <Tile t={t} priority={i < 3} sizes="(max-width: 640px) 50vw, 33vw" />
              </div>
            ))}
          </div>
        </div>
    ...
    <section ref={ref} className="relative h-[200vh] bg-[#07081a] text-white">
      <div className="sticky top-0 flex h-screen flex-col items-center justify-center gap-4 overflow-hidden px-4 sm:px-6 lg:px-8">
        <motion.div
          ...
          className="shrink-0 text-center will-change-transform"
        >
          <Heading />
        </motion.div>

        {/* Height-sized 3×3 grid — always fully visible, never clipped */}
        <div className="grid h-[72vh] w-full max-w-[1320px] grid-cols-3 grid-rows-3 gap-2.5 sm:gap-3">
          {TILES.map((t, i) => {
            ...
            return (
              <motion.div
                ...
                className="h-full min-h-0 transform-gpu will-change-transform"
              >
                <Tile
                  t={t}
                  priority={isFeatured}
        ...
        <motion.div
          ...
          className="absolute inset-x-0 bottom-5 z-40 flex flex-col items-center gap-2 text-white/70"
        >
